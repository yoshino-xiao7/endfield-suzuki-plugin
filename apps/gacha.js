...
export class GachaApp extends plugin {
    ...
    async records(e) {
        ...
        try {
            ...
            await this._enrichPlayerAvatarsWithWiki(playerInfo, records)
    ...
    async _enrichPlayerAvatarsWithWiki(playerInfo, records) {
        try {
            ...
            const names = [...new Set((records || [])
                ...
                .map(r => r.itemName)
                .filter(Boolean))]
            const missing = names.filter(name => !playerInfo.charAvatars[name])
            if (missing.length === 0) return playerInfo

            const wikiAvatars = await api.getWikiAvatarMap(missing)
            for (const [name, image] of Object.entries(wikiAvatars)) {
                if (!playerInfo.charAvatars[name]) playerInfo.charAvatars[name] = image
            }
        } catch (err) {
            logger.warn(`[Endfield] Wiki 头像补全失败: ${err.message}`)
        }
        return playerInfo
    }
}
